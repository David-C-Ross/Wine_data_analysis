...
X_trainval, X_test, y_trainval, y_test = train_test_split(df[cols], df['quality'], test_size=0.2, stratify=df['quality'], random_state=69)
...
sns.barplot(data = pd.DataFrame.from_dict([get_class_distribution(df['quality'])]).melt(), x = "variable", y="value", hue="variable",  ax=axes[2]).set_title('Class Distribution in Origninal Set')
...
print(normalized_mutual_info_score(y_test, y_pred_list))
